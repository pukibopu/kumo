...
// Surface-function template (MD): surface_write splices params into the
// MaterialFactors block and replaces the kumoSurface line; everything after
// main()'s surface call is the untouchable lighting tail.
...
// The surface function's whole world: sampled PBR defaults in, overrides out.
struct SurfaceInputs {
    vec3 worldPos;
    vec3 normal; // normal-mapped world normal
    vec2 uv;     // tiled
    vec3 viewDir;
    float time;     // seconds since renderer init
    vec4 baseColor; // texture * factor
    float metallic;
    float roughness;
    float occlusion;
    vec3 emissive;
...
void kumoSurface(inout SurfaceOutputs s, in SurfaceInputs i) {} //KUMO_SURFACE_FUNCTION
...
void main() {
    ...
    SurfaceInputs si;
    si.worldPos = vWorldPos;
    si.uv = uv;
    si.viewDir = normalize(frame.cameraPos.xyz - vWorldPos);
    si.normal = perturbNormal(normalize(vNormal), vWorldPos, uv);
    si.time = frame.timeParams.x;
    si.baseColor = baseSample;
    si.metallic =
        clamp(mrSample.b * material.metallicRoughness.x * frame.materialOverride.x, 0.0, 1.0);
    si.roughness =
        clamp(mrSample.g * material.metallicRoughness.y * frame.materialOverride.y, 0.045, 1.0);
    si.occlusion = texture(sampler2D(occlusionTex, materialSampler), uv).r;
    si.emissive = texture(sampler2D(emissiveTex, materialSampler), uv).rgb * material.emissive.rgb;

    SurfaceOutputs s;
    s.albedo = si.baseColor.rgb;
    s.alpha = si.baseColor.a;
    s.metallic = si.metallic;
    s.roughness = si.roughness;
    s.normal = si.normal;
    s.occlusion = si.occlusion;
    s.emissive = si.emissive;
    kumoSurface(s, si);

    // Lighting tail: identical to pbr.frag, fed from the surface outputs.
    vec3 albedo = s.albedo;
    float metallic = clamp(s.metallic, 0.0, 1.0);
    float roughness = clamp(s.roughness, 0.045, 1.0);
    float occlusion = clamp(s.occlusion, 0.0, 1.0);
    ...
    fragColor = vec4(direct + ambient + s.emissive, s.alpha);
}
