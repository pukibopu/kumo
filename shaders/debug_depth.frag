...
void main() {
    // timeParams.y carries the frame's max view depth; near = white, far = black.
    float viewDepth = -(frame.view * vec4(vWorldPos, 1.0)).z;
    float span = max(frame.timeParams.y, 1e-3);
    float shade = clamp(1.0 - viewDepth / span, 0.0, 1.0);
